...
_Last updated: 2026-06-27 — Phase 7 STARTED (backend stand-up sub-phase 7.0). Phase 6 COMPLETE._
...
1. **(IN PROGRESS) Sub-phase 7.0a + 7.0c dispatched** — `backend-engineer` builds `app/main.py` (app factory, router/exception/CORS/cookie/logging wiring) atop the existing scaffold; `testing-engineer` builds the pytest harness + fixtures. Both target the existing scaffold conventions and the frontend contract (`sis_refresh` cookie, `X-Refresh: 1`, credentialed CORS, `{access_token, user}` envelope).
2. **(HELD) Request the user's Supabase Postgres connection string + project-ref**, then run 7.0b (Alembic env + initial migration for the 28 tables + seed: Principal, school_profile, default grading scale/policy) and 7.0d (OQ-FE-A cross-origin cookie/CORS hard-reload smoke test). Store secrets only in a gitignored `.env`.
3. **7.0e** — wire orval against the served OpenAPI; replace FE-3 bridge types.
4. **Then D17 module order**, one at a time, backend-first: Auth → Settings → Students/Teachers → Classes → Assessments → Grades/Attendance → Announcements → Dashboard/Reports. Per module: build (backend-engineer + frontend-engineer) → test (testing-engineer) → review → update `frontend-implementation.md` + this tracker. Do NOT build ahead.
