...
mounted under `/api/v1`:
  ...
  * `categories_router` prefix /classes      — per-class_subject categories

Writes are teacher-only (+ ownership, enforced in the service). Reads are scoped.
Route order: /assessments/class-subjects is declared BEFORE /assessments/{id} so
...
categories_router = APIRouter(prefix="/classes", tags=["assessments"])
...
@router.get("/class-subjects", response_model=ClassSubjectPickerList,
...
def list_class_subjects_picker(
    ...
    return service.list_class_subjects_picker(
...
@router.post("", response_model=AssessmentDetail, status_code=status.HTTP_201_CREATED,
             summary="Create an assessment (teacher; owns class_subject; api-spec §6)",
...
_CAT = "/{class_id}/subjects/{class_subject_id}/categories"
...
def list_categories(
    ...
    return service.list_categories(
        db, caller=caller, class_id=class_id, class_subject_id=class_subject_id
    )
...
def create_category(
    ...
    return service.create_category(
        db, actor=actor, class_id=class_id, class_subject_id=class_subject_id, payload=payload
    )
