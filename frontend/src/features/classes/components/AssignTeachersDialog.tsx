...
import { useAssignTeachers } from '../hooks/useClasses';
import type { ClassSubjectItem, TeacherRef } from '../types';

/**
 * Assign teacher(s) to one subject offering (api-spec §5 PUT
 * /classes/{id}/subjects/{csId}/teachers). P/S only (the caller gates rendering).
 *
 ...
 *  - An empty set is legal and removes all teachers. That is offered deliberately (an
 *    explicit warning + a "Remove all teachers" submit label), never as a silent accident.
 *
 * The picker reads the paginated Teachers directory, so it pages rather than truncating —
 * every teacher is reachable via search or the pager. Selections are held separately from
...
export interface AssignTeachersDialogProps {
  ...
  classSubject: ClassSubjectItem | null;
  onClose: () => void;
  /** Called after a successful save so the caller can toast the outcome. */
  onAssigned: (result: { subjectName: string; count: number }) => void;
}
...
export function AssignTeachersDialog({
  ...
  const [offering, setOffering] = useState<ClassSubjectItem | null>(classSubject);
  useEffect(() => {
    if (classSubject) setOffering(classSubject);
  }, [classSubject]);
  ...
  const assignMut = useAssignTeachers(classId);
  ...
  const handleSubmit = () => {
    ...
    assignMut.mutate(
      {
        classSubjectId: offering.class_subject_id,
        body: { teacher_ids: selectedIds, lead_teacher_id: lead },
      },
      {
        onSuccess: (updated) => {
          onAssigned({
            subjectName: updated.subject.name,
            count: updated.teachers.length,
          });
          onClose();
        },
        onError: (err) => {
  ...
  const submitLabel = willClearAll
    ? 'Remove all teachers'
    : selected.length > 0
      ? `Save ${selected.length} teacher${selected.length > 1 ? 's' : ''}`
      : 'Save teachers';

  return (
    <FormDialog
      open={open}
      title={`Teachers for ${subjectName}`}
    ...
    >
      {selected.length > 0 ? (
        ...
        >
          <FormLabel component="legend" sx={{ typography: 'subtitle2' }}>
            Assigned ({selected.length}) — choose the lead teacher
          </FormLabel>
          <RadioGroup value={lead ?? ''} onChange={(e) => setLead(e.target.value)}>
            {selected.map((t) => (
              ...
              >
                ...
                <IconButton
                  size="small"
                  aria-label={`Remove ${t.full_name} from ${subjectName}`}
                  onClick={() => toggle(t)}
                >
        ...
        <Alert severity="warning" sx={{ mb: 2 }}>
          Saving now removes every teacher from {subjectName}. It will show as “Needs teacher”
          until someone is assigned again.
        </Alert>
      ) : (
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          No teacher is assigned to {subjectName} yet. Select one or more below — the first one
          you pick becomes the lead.
        </Typography>
      )}
      ...
      {listQuery.isError ? (
        ...
        <LoadingState variant="form" rows={4} label="Loading teachers" />
      ) : teachers.length === 0 ? (
        <EmptyState
          title={debouncedSearch ? 'No matching teachers' : 'No active teachers'}
          description={
            debouncedSearch
              ? 'Try a different name or staff number.'
              : 'Add a teacher in the Teachers module before staffing this subject.'
          }
          variant="card"
        />
      ) : (
        <>
          <List dense aria-label={`Teachers available for ${subjectName}`}>
            {teachers.map((t) => {
              const checked = selected.some((s) => s.id === t.id);
              return (
                <ListItemButton
                  key={t.id}
                  onClick={() => toggle({ id: t.id, full_name: t.full_name })}
                  role="checkbox"
          ...
          {totalPages > 1 && (
            ...
            >
              ...
              <Stack direction="row" spacing={0.5}>
                <IconButton
                  size="small"
                  aria-label="Previous page of teachers"
                ...
                <IconButton
                  size="small"
                  aria-label="Next page of teachers"
