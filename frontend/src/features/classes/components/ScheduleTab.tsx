...
import { useClassMeetings, useReplaceMeetings } from '../hooks/useClasses';
import { DAY_LONG, formatTimeRange, meetingRowInvalid } from '../meetingFormat';
import { MeetingRowsEditor } from './MeetingRowsEditor';
import type { ClassMeetingInput, ScheduleConflict } from '../types';

/**
 * Class detail → Schedule tab. The class's Mon–Fri meeting rows (FR-SCH-01/02).
 *
 * The write is `PUT /classes/{id}/meetings`, which REPLACES the whole week, so this edits a
 * local draft and saves it in one request. That mirrors the API rather than fighting it: a
 * retimed week cannot half-apply, and there is no per-row save button to leave the screen in
 * a partially-written state.
 *
 * **Conflicts are warnings, not errors.** A successful save can still return teacher/room
 ...
 *
 * Read-only for a viewer who cannot manage classes: teachers and students see when their
 * class meets, only the office changes it.
 */
...
function toDraft(
  ...
  return meetings.map((m) => ({
    day_of_week: m.day_of_week as ClassMeetingInput['day_of_week'],
...
function sameDraft(a: ClassMeetingInput[], b: ClassMeetingInput[]): boolean {
  return JSON.stringify(a) === JSON.stringify(b);
}

export function ScheduleTab({ classId, className, canManage }: ScheduleTabProps) {
  const query = useClassMeetings(classId);
  const saveMut = useReplaceMeetings(classId);

  const server = useMemo(() => toDraft(query.data?.meetings ?? []), [query.data]);
  const [draft, setDraft] = useState<ClassMeetingInput[]>([]);
    ...
    if (draft.length === 0) {
      return (
        <EmptyState
          ...
          description={`${className} does not have meeting times set. The office adds them.`}
        />
      );
    }
    return (
  ...
  return (
    <Stack spacing={2}>
      <Typography variant="body2" color="text.secondary">
        When {className} meets each week. Students see this on their timetable.
      </Typography>
      ...
      {draft.length === 0 && (
        <Typography variant="body2" color="text.secondary">
          No meetings yet. Add one to put this class on students' timetables.
        </Typography>
      )}
