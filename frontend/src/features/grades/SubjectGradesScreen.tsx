...
/**
 * Grades drill-down for ONE subject (reached by clicking a subject card). Lists the
 * subject's assessments; clicking an assessment opens the per-assessment class grading
 * page. Teachers who own the offering author here too — create / edit / status / delete —
 * so assessment management is folded into Grades (the standalone Assessments nav is gone
 * for staff). Grade entry itself stays on AssessmentGradingScreen.
 */
export function SubjectGradesScreen({
  ...
  option: ClassSubjectOption | null;
  ...
  const listParams = useMemo(
    () => ({ class_subject_id: classSubjectId, page: page + 1, page_size: pageSize, sort: '-assessment_date' }),
    [classSubjectId, page, pageSize],
  );
  const listQuery = useAssessmentsList(listParams, Boolean(classSubjectId));
  ...
  const openGrading = (a: AssessmentListItem) =>
    navigate(`${ROUTES.gradeAssessment}/${a.id}?${CLASS_SUBJECT_PARAM}=${encodeURIComponent(classSubjectId)}`);
  ...
  return (
    <Box sx={{ pt: 3 }}>
      ...
      <PageHeader
        title={option?.subject?.name ?? 'Subject'}
      ...
      {listQuery.isError ? (
        ...
        <DataTable<AssessmentListItem>
          caption="Assessments for this subject"
          columns={columns}
          ...
          emptyDescription={
            canAuthor ? 'Create the first assessment for this subject.' : 'No assessments to show.'
          }
