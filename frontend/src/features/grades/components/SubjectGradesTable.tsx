...
export interface SubjectGradesTableProps {
  /** The subject whose released assessments are shown (student scope). */
  subject: MyGradeSubject;
}

/**
 * The per-subject released-grades table for the student "My Grades" view
 ...
 * subject has no released grades yet. Read-only; the server already applies the
 * release filter, so unreleased assessments are never in the payload.
 */
export function SubjectGradesTable({ subject }: SubjectGradesTableProps) {
  const subjectName = subject.class_subject?.subject?.name ?? 'Subject';
