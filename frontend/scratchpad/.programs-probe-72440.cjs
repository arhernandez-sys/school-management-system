...
__export(programs_exports, {
  programsHandlers: () => programsHandlers
});
module.exports = __toCommonJS(programs_exports);
...
function curriculumRows(programId2) {
  return D.program_courses.filter((pc) => pc.program_id === programId2);
}
...
var programsHandlers = [
  import_msw2.http.get(`${API_BASE_URL}/programs`, ({ request }) => {
  ...
  import_msw2.http.get(`${API_BASE_URL}/programs/:programId`, ({ params }) => {
    const program = findProgram(String(params.programId));
    if (!program) return errorResponse(404, "not_found", "Program not found.");
    return import_msw2.HttpResponse.json(detail(program));
  }),
  import_msw2.http.post(`${API_BASE_URL}/programs`, async ({ request, cookies }) => {
    ...
    return import_msw2.HttpResponse.json(detail(created), { status: 201 });
  }),
  import_msw2.http.patch(`${API_BASE_URL}/programs/:programId`, async ({ params, request, cookies }) => {
    const denied = assertDean(cookies);
    if (denied) return denied;
    const program = findProgram(String(params.programId));
    if (!program) return errorResponse(404, "not_found", "Program not found.");
    const body = await request.json();
    if (body.code && D.programs.some((p) => p.id !== program.id && p.code.toLowerCase() === body.code.toLowerCase())) {
      return errorResponse(409, "duplicate_program_code", "A program with this code already exists.");
    }
  ...
  import_msw2.http.delete(`${API_BASE_URL}/programs/:programId`, ({ params, cookies }) => {
    const denied = assertDean(cookies);
    if (denied) return denied;
    const program = findProgram(String(params.programId));
    if (!program) return errorResponse(404, "not_found", "Program not found.");
  ...
  // ── Curriculum ────────────────────────────────────────────────────────────────
  import_msw2.http.post(`${API_BASE_URL}/programs/:programId/courses`, async ({ params, request, cookies }) => {
    const denied = assertDean(cookies);
    if (denied) return denied;
    const program = findProgram(String(params.programId));
    if (!program) return errorResponse(404, "not_found", "Program not found.");
    ...
    return import_msw2.HttpResponse.json(detail(program), { status: 201 });
  }),
  import_msw2.http.patch(
